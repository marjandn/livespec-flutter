...
main() {
  ...
  test(
    'should return parsed Swagger model when getSwaggerLinkJsonData is called with valid link',
    () async {
      ...
      final result = await swagerRemoteDataSource.getSwaggerLinkJsonData(
        validLink,
      );
  ...
  test(
    'should throw ParseException when getSwaggerLinkJsonData is called given invalid JSON data',
    () async {
      const errorMessage = 'Something went wrong';
      final mockClient = MockClient(
        (request) async => Response(errorMessage, 500),
      );
      final swagerRemoteDataSource = SwaggerRemoteDatasourceImpl(mockClient);
      ...
      try {
        ...
        expect(
          e,
          isA<JsonParsingException>().having(
            (e) => e.message,
            'message',
            errorMessage,
          ),
  ...
  test(
    'should throw RequestExceptions when getSwaggerLinkJsonData is called with invalid link',
    () async {
      const errorMessage = 'Invalid link';
      final mockClient = MockClient(
        (request) async => Response('Invalid link', 400),
      );
  ...
  test(
    ...
    () async {
      final response = jsonEncode({'mockBaseUrl': 'https://sampleswagger.com'});
      ...
      final result = await swaggerRemoteDatasource.generateSwaggerMock(
        validLink,
      );

      expect(
        result,
        isA<MockedSwaggerResponse>().having(
          (e) => e.mockBaseUrl,
          'Base URL',
          'https://sampleswagger.com',
        ),
      );
    },
  );
  ...
  test(
    ...
    () async {
      ...
      try {
        await swaggerRemoteDatasource.generateSwaggerMock(invalidLink);
      } catch (e) {
        expect(
          e,
          isA<RequestExceptions>().having(
